...
const MIGRATION_SQL = `CREATE TABLE entries (id INTEGER PRIMARY KEY AUTOINCREMENT, url TEXT NOT NULL UNIQUE, title TEXT NOT NULL, description TEXT NOT NULL, read INTEGER NOT NULL DEFAULT 0, source_type TEXT NOT NULL DEFAULT 'generic', created_at TEXT NOT NULL DEFAULT (datetime('now')), updated_at TEXT NOT NULL DEFAULT (datetime('now'))); CREATE TABLE tags (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT NOT NULL UNIQUE); CREATE TABLE entry_tags (entry_id INTEGER NOT NULL REFERENCES entries(id) ON DELETE CASCADE, tag_id INTEGER NOT NULL REFERENCES tags(id) ON DELETE CASCADE, PRIMARY KEY (entry_id, tag_id)); CREATE INDEX idx_entries_read ON entries(read); CREATE INDEX idx_entries_created_at ON entries(created_at DESC); CREATE VIRTUAL TABLE entries_fts USING fts5(title, description, content='entries', content_rowid='id'); CREATE TRIGGER entries_ai AFTER INSERT ON entries BEGIN INSERT INTO entries_fts(rowid, title, description) VALUES (new.id, new.title, new.description); END; CREATE TRIGGER entries_ad AFTER DELETE ON entries BEGIN DELETE FROM entries_fts WHERE rowid = old.id; END; CREATE TRIGGER entries_au AFTER UPDATE ON entries BEGIN UPDATE entries_fts SET title = new.title, description = new.description WHERE rowid = new.id; END; CREATE TABLE notes (id INTEGER PRIMARY KEY AUTOINCREMENT, entry_id INTEGER NOT NULL REFERENCES entries(id) ON DELETE CASCADE, content TEXT NOT NULL, page_number TEXT, created_at TEXT NOT NULL DEFAULT (datetime('now')), updated_at TEXT NOT NULL DEFAULT (datetime('now'))); CREATE INDEX idx_notes_entry_id ON notes(entry_id); CREATE TABLE schema_version (version INTEGER PRIMARY KEY); INSERT INTO schema_version (version) VALUES (3);`
...
describe('GET / — view parameter (integration)', () => {
  ...
  function insertEntry(url: string, title: string, description: string, read: number): void {
    const db = getDatabase(dbPath)
    db.prepare(`INSERT INTO entries (url, title, description, read, source_type) VALUES (?, ?, ?, ?, 'generic')`).run(url, title, description, read)
  }

  function insertTaggedEntry(url: string, title: string, description: string, read: number, tag: string): void {
    const db = getDatabase(dbPath)
    db.prepare(`INSERT INTO entries (url, title, description, read, source_type) VALUES (?, ?, ?, ?, 'generic')`).run(url, title, description, read)
